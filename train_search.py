...
import utils
from utils import calc_z_shapes, likelihood_loss, likelihood_loss_variance
...
# Arguments
parser = argparse.ArgumentParser(description='NADS')
...
parser.add_argument('--save', type=str, default='EXP-MNIST', help='experiment name')
...
def sample_mnist(batch_size, image_size):
  ...
  dataset = torchvision.datasets.MNIST("mnist", transform=transform, target_transform=None, download=True)
...
def main():
  ...
  dataset = iter(sample_mnist(args.batch, args.img_size))
  ...
  with tqdm(range(args.iter)) as pbar:
    for i in pbar:
      ...
      loss_variance = likelihood_loss_variance(log_p, logdet, args.img_size, n_bins, approx_samples)
      ...
      if i % 1000 == 0:
        ...
        torch.save(
          model.state_dict(),
          "{}/checkpoint/model_{}.pt".format(args.save, str(i + 1).zfill(6)) 
        )
